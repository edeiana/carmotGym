...
To ensure the accuracy of execution time measuraments, all frequency scaling mechanisms (including TurboBoost) have to be disabled and the machine must be idle (no other compute or memory intensive process can run on the machine during the execution of the experiments).
...
If you are allowed to use SPEC2017 for artifact evaluation purposes (please check if your license allows to do so) and you'd like to generate the results for SPEC2017, you will have to include it manually in the podman container (see Section below to know how).
...
This will open a shell inside the podman container (if you are reading this README.md you probably have already done that).
...
The customization of the experiments can be disabled by unsetting the corresponding environment variables (e.g., unset CARMOT_NUM_RUNS ; unset CARMOT_FULL).

The progress of the experiments can be monitored by looking at ~/carmot_experiments_output.txt:
